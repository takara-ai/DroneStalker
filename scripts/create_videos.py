...
Creates two videos:
1. colored.webm - from RGB frames
2. motion.webm - from Event frames
"""
...
# Configuration
SEQUENCE_FOLDER = "230"  # Change this to select different sequence
DATA_DIR = Path(__file__).parent.parent / "data"
OUTPUT_DIR = DATA_DIR / SEQUENCE_FOLDER

# Frame directories
RGB_DIR = OUTPUT_DIR / "RGB"
EVENT_DIR = OUTPUT_DIR / "Event" / "Frames"

# Output video files
COLORED_VIDEO = OUTPUT_DIR / "colored.webm"
MOTION_VIDEO = OUTPUT_DIR / "motion.webm"

# Video settings
FPS = 30  # Frames per second
VIDEO_CODEC = "libvpx-vp9"  # WebM codec
QUALITY = "good"  # Quality preset: good, best, realtime
SPEED = 1  # Encoding speed (0-5, lower = better quality but slower)
...
CRF = 40  # Quality (for VP9: 0-63, lower = better quality, higher = smaller files)
...
def parse_event_timestamp(filename: str) -> Optional[float]:
    ...
    Format: Video_230_100032333.png -> 100032333 (microseconds)
    Returns: seconds (microseconds / 1_000_000)
    """
    # Extract the numeric timestamp
    match = re.search(r"Video_\d+_(\d+)\.png", filename)
    if not match:
        return None
...
def main():
    ...
    print("=" * 60)
    print("FRED Dataset Video Creator")
    print("=" * 60)
    ...
    # Create RGB video (colored.webm)
    print("\n" + "=" * 60)
    print("Creating RGB video (colored.webm)")
    print("=" * 60)
    success_rgb = create_video(RGB_DIR, COLORED_VIDEO, "jpg", is_rgb=True, fps=None)

    # Create Event video (motion.webm)
    print("\n" + "=" * 60)
    print("Creating Event video (motion.webm)")
    print("=" * 60)
    ...
    if success_rgb:
        ...
        print(f"✓ colored.webm created ({size:.2f} MB)")
    else:
        print("✗ Failed to create colored.webm")

    if success_event:
        size = MOTION_VIDEO.stat().st_size / (1024 * 1024)  # MB
        print(f"✓ motion.webm created ({size:.2f} MB)")
    else:
        print("✗ Failed to create motion.webm")
